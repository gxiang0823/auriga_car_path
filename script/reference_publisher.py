...
def generate_center_line(config, start_x=0.0, start_y=0.0, start_yaw=0.0, resolution=0.1, left_bound=6.0,
                         right_bound=2.5):
    x = start_x
    y = start_y
    yaw = start_yaw
...
def generate_random_dynamic_vehicles(center_line, count, horizon=40.0, dt=0.1):
    ...
    velocities = 2 + 6 * np.random.rand(count) #Generate count of random numbers between 0 and 1
    ...
    for i in range(count):
        ...
        rand_lateral = 4.0 if np.random.rand() > 0.5 else 0.0
...
def main():
    ...
    if 'dynamic' in sys.argv or 'pedestrian' in sys.argv:
        ...
        if 'pedestrian' in sys.argv:
            dynamic_obstacles.obstacles.extend(generate_random_pedestrian(center_line, 10).obstacles)

        if 'dynamic' in sys.argv:
            dynamic_obstacles.obstacles.extend(generate_random_dynamic_vehicles(center_line, 4).obstacles)
