...
double TrajectoryNLP::SolveIteratively(double w_inf, const Constraints &constraints, const States &guess,
  ...
  auto identity = DM::ones(config_.nfe, 1);//构造一个全是1的nfe行1列的矩阵

  //整个时间序列内，所有参数的上下限设置为nfe行1列的矩阵，接下来初始化中间值及起、末信息
  ...
  // boundary value constraints：起终点的限制
  ...
  lb_x(end) = ub_x(end) = constraints.end_x;
  lb_y(end) = ub_y(end) = constraints.end_y;
  lb_theta(end) = ub_theta(end) = constraints.end_theta;
  ...
  arg["p"] = DM::vertcat({w_inf, ref_x, ref_y, ref_theta});//不清楚这里的w_inf是什么？似乎是对迭代的处罚权重
