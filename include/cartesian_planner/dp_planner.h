...
public:
  ...
  bool Plan(double start_x, double start_y, double start_theta, double end_x, double end_y, double end_theta, DiscretizedTrajectory &result);
...
private:
  ...
  double GetLateralOffset(double s, int l_ind) {
    // if (l_ind == NL - 1) {
    //   std::cout << s <<", "<< lb + (ub - lb) * lateral_[l_ind]<<std::endl;
    //   return 0.0;
    // }

    auto ref = env_->reference().EvaluateStation(s);//利用插值法获得s坐标对应的TrajectoryPoint信息
    double lb = -ref.right_bound + safe_margin_;
    double ub = ref.left_bound - safe_margin_;

    // if (l_ind == NL - 1) {
    //   double l = lb + (ub - lb) * lateral_[l_ind];
    //   auto xy = env_->reference().GetCartesian(s, l);
    //   std::cout << s <<", "<< l<<". "<< xy.x() << ", "<< xy.y()<<std::endl;
    //   return 0.0;
    // }
